...
public class ChatFormat {
    ...
    public ChatFormat(MegaChat megaChat, String groupName, ConfigurationSection formatSection) {
    ...
    public String getFormattedJsonMessage(Player sender) {
        String formattedFormat = color(format);

        //todo add placeholder api support.

        //channel
        TextComponent.Builder channelFormat = TextComponent.builder(color(channel));
        if (!channel_tooltip.equalsIgnoreCase("")) {
            channelFormat.hoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.of(color(channel_tooltip))));
        }
        ...
        //prefix
        TextComponent.Builder prefixFormat = TextComponent.builder(color(prefix));
        if (!prefix_tooltip.equalsIgnoreCase("")) {
            prefixFormat.hoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.of(color(prefix_tooltip))));
        }
        ...
        //name
        TextComponent.Builder nameFormat = TextComponent.builder(color(name.replace("%username%", sender.getName()).replace("%displayname%", sender.getDisplayName())));
        if (!name_tooltip.equalsIgnoreCase("")) {
            prefixFormat.hoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.of(color(name_tooltip))));
        }
        ...
        //suffix
        TextComponent.Builder suffixFormat = TextComponent.builder(color(suffix));
        if (!suffix_tooltip.equalsIgnoreCase("")) {
            prefixFormat.hoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.of(color(suffix_tooltip))));
        }
